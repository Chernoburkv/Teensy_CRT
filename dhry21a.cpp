...
/*
 *************************************************************************
 *	This DHRYSTONE benchmark was adapted to Arduino by 'monsonite' and was
 *	found at http://stm32duino.com/viewtopic.php?t=76
 *
 *  Modified here to further to make measurements on TEENSY-3.6 platform
 *	by making one callable test routine so it could be incorporated into
 *	other programs and test sequences.
 *
 *	20180427	Rev 0.0	E. Andrews Initial Cut
 *
 *************************************************************************
 *  Based on original work by Reinhold P. Weicker
 *************************************************************************
 *
...
/* 
	...
	void setup() {
	  ...
	  // Initializations 
		Serial.begin(115200); 	//Arduino Serial Init
	  while (!Serial) {
	...
	void loop() {  
		while (AllDone==0){
			Run_DHRYSTONE_benchmark(2000000);
			AllDone=2;
		}
	}
*/
double Run_DHRYSTONE_benchmark(double Num_Of_Runs){
  ...
  Serial.println(); Serial.println("======== DHRYSTONE SUBROUTINE RUN ========== ");
 ...
 /*  
	  ...
	  Serial.print(" runs through Dhrystone...");
   */
...
/*	Detailed printout of internal variables
  printf ("\r\n");
...
*/
  ...
  {
    DhryCycleTime_us = (double) User_Time / (double) Number_Of_Runs;
